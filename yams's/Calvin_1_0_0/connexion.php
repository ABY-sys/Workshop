...
if(isset($_POST['formconnec']))
{
	$mail = htmlspecialchars($_POST['mail']);
	$motdepasse = sha1($_POST['motdepasse']);
	if(!empty($mail) AND !empty($motdepasse))
	{
           $requser = $bdd ->prepare("SELECT * FROM espace WHERE mail = ? AND motdepasse = ?");
...
<body>
  <div class="column large-8 intro" style="margin-left: auto; margin-right: auto;">
 	...
 	<form method="POST" action="">
       	...
       	<input type="email" name="mail"  placeholder="mail" class="h-full-width">
        <input type="password" placeholder=" mot de passe"  name="motdepasse"class="h-full-width">
		<input type="submit" name="formconnec" value="se connecter" class="btn btn--primary btn-wide btn--large h-full-width">


<div class="row justify-content-center my-2"> <a href="#"><small class="text-muted">Mot de passe oublié?</small></a> </div>
                    </div>
                </div>
                <div class="bottom text-center mb-5" style="margin-left: auto; margin-right: auto;" >
                    <a href="inscription.php" class="sm-text mx-auto mb-3">Vous avez pas de compte?<button class="btn btn-white ml-2">S'inscrire</button></a>
                </div>
